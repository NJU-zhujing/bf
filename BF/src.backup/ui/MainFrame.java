			...
			if (cmd.equals("Open")) {
				if(LoginFrame.userId!=null){
					try {	
						...
						try{
							label01=new JLabel(help[length-1]);
							label02=new JLabel(help[length-2]);
							label03=new JLabel(help[length-3]);
							label04=new JLabel(help[length-4]);
...
//			}
	...
	class SaveActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(LoginFrame.userId!=null){
			try {
				RemoteHelper.getInstance().getIOService().writeFile(code, LoginFrame.userId, inputTextArea.getText());		 
			} catch (RemoteException e1) {
				e1.printStackTrace();
			}
		}else{
